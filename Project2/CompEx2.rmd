...
with $\lambda>0$ being a tuning parameter. From before we know that

\begin{equation}
  RSS = \sum_{i=1}^n \left(y_i - \hat\beta_0 - \sum_{j=1}^p \hat\beta_j x_{ij} \right)^2
\end{equation}

This can be written in terms of matrices and vectors as
...
Where $I$ is the identity matrix. Even though $I$ is an identity matrix the first element would have to be 0 in order to not penalize the intercept $\beta_0$ with the ridge regression. Maybe this type of matrix has a name?

## b)

Already know that the expectation value of $y=X\beta + \epsilon$ is $\operatorname{E}[y] = X\beta$, as $\operatorname{E}[\epsilon]=0$. The expectation value of $\beta_{Ridge}$ is then
...
The obvious choice might be the model with 14 predictors, as this had both the highest adjusted $R^2$ and the smallest $C_p$. However, since the improvement is very small for the larger models, we will choose the model with 6 variables, as this has the smallest BIC, and BIC is defined in a way that normally favors a smaller model. Cross validation also shows that 6 would be a good choice. It is not the one with the lowest mean error, but it is quite good compared to the rest. The model with 6 variables has a MSE of `r round(MSE.forward, 3)`.
...
The relationship between `Outstate` and `Room.Board` seems to be approximately linear, same for `perc.alumni`. For `Terminal` on the other hand the slope seems to increase with increasing value for `Terminal`. The relation between `Outstate` and `Expend` does not seem linear, however the relation between `Outstate` and `Grad.Rate` does.
...
legend("topright", inset=c(-0.32,0.2), legend = paste("d =",1:degs), col = c(1:degs), lty = 1, bty="n")
```
...
summary(d.train)
plot(d.train)
plot(diabetes~glu+bmi, data = d.train)
plot(bmi~skin, data = d.train)
plot(log(d.train$npreg))
```
...
(ii) True
(iii) True
(iv) True? Mye flere datapunkter langt nede, log(npreg) gir en jevnere fordeling
